 ...
 * VitePress: ::: tip 注意\n内容\n:::
 */
export class QuoteBlocksTransformer implements MarkdownTransformer {
  name = 'quoteBlocks';
   ...
   * 处理引用块内容
   */
  private processQuoteBlock(quoteContent: string[]): string[] {
   ...
   * 检查是否为警告框格式
   */
  private isAlertBox(line: string): boolean {
    const alertPatterns = [
      /\*\*注意[：:]\*\*/,
      /\*\*警告[：:]\*\*/,
      /\*\*错误[：:]\*\*/,
